        ...
        <div className={cx(`${isDarkMode ? 'post-theme-dark' : ''}`, 'post__container')}>
            ...
            <Modal size="sm" centered show={modal} onHide={() => setModal(!modal)}>
                <ModalBody bsPrefix="modal-custom">
                    {userInfo.username && data.user.username && userInfo.username === data.user.username && (
                        <div className={cx('more-action')}>Xóa bài đăng</div>
                    )}
            ...
            <div>
                <div style={{ marginBottom: '15px', display: 'flex', padding: '0 1rem' }}>
                    ...
                    />
                    <RiShareForwardLine size="25px" className={cx('post__reactIcon')} />
                </div>
                <div style={{ fontSize: '14px', marginLeft: '12px', marginBottom: '10px', fontWeight: '100' }}>
                    {data.countReaction[6]} {data.countReaction[6] > 1 ? 'Likes' : 'lượt thích'}
                </div>
            </div>
